...
def showimage8(images, unnormlize=2047.0, first_channel=False):
    ...
    if isinstance(images, torch.Tensor):
        # maxval = np.float32(torch.max(images))
        # unnormlize = np.where(max(np.float32(torch.max(images)), 1.0) > 1.0, 1.0, unnormlize)
        if first_channel:
            images = images.permute(1, 2, 0)
        if images.shape[2] == 8:
            output = images[..., [0, 2, 4]] * torch.tensor(unnormlize)  # BGR
        elif images.shape[2] == 4 or images.shape[2] == 3:
            output = images[..., [0, 1, 2]] * torch.tensor(unnormlize)  # BGR
        elif images.shape[2] == 1:
            output = images[..., [0, 0, 0]] * torch.tensor(unnormlize)  # gray
        output = torch.clamp(output, 0, int(unnormlize))
