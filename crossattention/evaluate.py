...
def analysis_accu(img_base, img_out, ratio, flag_cut_bounds=False, dim_cut=21, choices=4):
    ...
    if choices == 5:
        ...
        CC = C1 / ((C2 * C3) ** 0.5)
        CC = torch.mean(CC)
        return {'SAM': SAM, 'ERGAS': ERGAS, 'PSNR': PSNR, 'SSIM': SSIM, 'CC': CC, 'PSNR_C' : channel_psnr}  # , q2n_index
...
if __name__ == "__main__":
    ...
    h, w = [64, 64]
    ms = torch.rand(1, 8, h*4, w*4).cuda()
    gt = torch.rand(1, 8, h*4, w*4).cuda()

    print(analysis_accu(ms, gt, ratio=4, dim_cut=0, choices=5))